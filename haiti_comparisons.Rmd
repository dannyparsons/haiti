...
---
## summary
Overall this data tells us that the total yearly rainfall is quite well estimated on average by CHIRPS, but there's little correlation between yearly values.We notice that Outside of the hurricane season, CHIRPS seems to miss most of the rainfall events. However, Within the hurricane season the estimates of total seasonal rainfall are quite good though the number of rain days is overestimated which is expected from chirps.The bias in the number of rain days in the hurricane season looks to be highly seasonally and spatially dependent. If we were to correct this bias we might need to take a closer look at month and location dependent corrections.
...
Our hurricane season runs from June to November.We notice that in the graph for number of rainy days the (absolute) bias increases for the stations cap haitien, quanaminthe , jeremie , damien, petitionville and it decreases in les cayes and jacmel. We should note that les cayes and jacmel are very similar to each other in latitude so perhaps looking at further southern station values to see if this pattern is consistent would prove valuable. 
For total rainfall what we see is that the (absolute) bias is lower for all stations in the hurricane season except petitionville.This suggests that in the hurricane season CHIRPS is a better estimate of the average total rainfall over that period but not the number of raindays. If we look at the monthly graphs below what we should observe is that the biases for the months June - November should be lower for total rainfall in the stations mentioned and number of rain days should have a bias that is higher in the stations mentioned to explain this trend.  
-
...
The n represents the number of years of comparison between CHIPRS and station values, THE bias tells us how much the average rainfall between station and CHIRPS vales differ and cor tell the correlation between these values.rSD stands for relative standard deviation. Correlation between the station and chirps value goes from an (absolute) low of 0.01 for petitionville to a high  of 0.67 for les cayes.We not that correlation is on the Low side for most stations. This likely means that we would have difficulty using the CHIRPS as a predictor of rain throughout the year.The (absolute) bias is quite low for the stations: cap Haitian , Quanaminthe, Jeremie and Petitionville, with all of them within a 100 of the station value. It is highest for les cayes which has a bias of -680. This data suggests might be able to use the chirps data with the stations that have a lower bias to determine how much rainfall there is across the whole year. It is worth  noting that Jacmel has 38 years of comparison and Jeremie has only 18 with the rest of the stations distributed between those 2 values. 
...
Correlation on the whole is very low between CHIRPS and station values across all the stations.The bias is quite low for the same sort of stations when we looked at total rainfall(cap Haitian,quanaminthe,Jeremie,Damien,petitionille). 
...
All the stations have a model for rain that is roughly the same so that the peaks and troughs occur at the same point in the year.While the station rain is predicted to have two peaks, the chirps model looks like it has one peak in Aug-Sep.For most stations the probabilities tend to be closer for the months Oct-Dec. Threshold 5 seems to improve the fit to the station for quanaminthe and cap haitian however it does not make much of a difference to the remaining stations. 
