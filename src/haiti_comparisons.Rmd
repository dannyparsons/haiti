...
zm <- zm %>%
  ...
  mutate(t_rain = sum(rain, na.rm = TRUE), #for each station and each month we calculate the total rain
         n_na = sum(is.na(rain)), #for each station and each month we calculate the number of missing values 
         # Make rain_adj = NA if in hurricane season, monthly total was 0 and there were no missing values
         # and otherwise rain_adj = rain
         rain_adj = ifelse(month %in% 6:11 && t_rain == 0 && n_na == 0, NA, rain))

view (zm)

zm[["rain_adj"]] <- ifelse(!is.na(zm[["rain_adj"]]), zm[["rain"]], zm[["rain_adj"]])

#in r vectors are generally vectorised so that you do not need a for loop 
...
zm_monthly_zero <- zm %>%
  group_by(station, month,year) %>%
  summarise(t_rain = sum(rain_adj, na.rm = TRUE),
            n_na = sum(is.na(rain_adj))) %>%
  filter(t_rain == 0)
View(zm_monthly_zero)
...
stations <- c( "cap_haitien","quanaminthe", "jeremie", "damien", "petitionville", "jacmel", "les_cayes") #north to south would be a better order for these 
...
gof_pr <- gof_syear %>% 
  unnest(cols = data)
gof_pr <- filter(gof_pr, station != "NA") #this line is used to get rid of the NA table , yet to determine where it comes from however
...
- In general some stations (quanaminthe , les cayes,cap haitien) have some correlation whereas the remaining have little correlation. 
-The mean is estimated best by cap haitien/petitionville and worst by les cayes/Jacmel. 
- its interesting to note that les cayes has the highest absolute bias but largest correlation. 
- Station and chirps values for quanaminthe and jeremie are compared for significantly fewer years than the other stations. 
-The relative standard deviation is highest in petitionville and the lowest in Damien
...
-Very little correlation overall with ranges from -0.25 to 0.35 with many stations closer to 0
- The first 4 or so stations have little positive bias so the chirps data overestimates the station data whereas the last three have slightly higher absolute bias though it is more negative. 
...
gof_pr_month <- filter(gof_pr_month, station != "NA")
view (gof_pr_month)  
#%>% 
  #group_by(product) %>% 
  n#est()
...
-The correlation between the monthly values is mch better than the yearly values. Les cayes and petitionville have high correlation in general. 
-
...
- Correlation sits between -0.08 and 0.45 for all values. Highest correlation is for quanaminthe and the lowest for haitien 
-
...
monthly_plots(gof_pr_month, 
                        ...
                        stat_pr = "n_rain__chirps_rain", 
                        stat_st = "n_rain__station", product_name = "chirps") #product_name = a is a dummy, ask danny variable at the momment
...
view(zm_long)
view (zm_long_st)
zambia_markov <- zm_long_st %>%   #zm_long_st does not have a column name rain, zm does have something similar (chirps vs rain)
  filter(!is.na(pr_rain)) %>% #is.na(rain) &
...
prop_amounts <- zm_long %>%
  filter(rain > 0.85 & station == "quanaminthe") %>%  #changed station name from "livingstone" to quanaminthe, else ita ll works all right 
