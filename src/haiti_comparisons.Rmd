...
---
##summary
- Total yearly rainfall is quite well estimated on average by CHIRPS, but there's little correlation between yearly values.
- Outside of the hurricane season, CHIRPS seems to miss most of the rainfall events. Within the hurricane season the estimates of total seasonal rainfall and quite good, and the number of rain days is overestimated (as expected)
- The bias in the number of rain days in the hurricane season looks to be highly seasonally and spatially dependent. If we want to correct this bias we might need month and location dependent corrections.
...
zm <- readRDS(here("src", "data", "daily_data_sorted1.RDS"))
...
metadata_station <- read.csv(here("src", "data", "daily_metadata.csv"))
...
hti_admbnda_adm0_cnigs_20181129 <- 
  sf::st_read(quiet=TRUE, 
              dsn = here("src", "data", "hti_adm_cnigs_20181129",  
...
locs <- read.csv(here("src", "data", "daily_metadata.csv"))
width <- 0.05
...
-Our hurricane season runs from June to November 
-We notice that in the graph for number of rainy days the (absolute) bias increases for the stations cap haitien, quanaminthe , jeremie , damien, petitionville and it decreases in les cayes and jacmel. We should note that les cayes and jacmel are very similar to each other in latitude
-For total rainfall what we see is that the (absolute) bias is lower for all station in the hurricane season except petitionville. 
-This suggests that in the hurricane season chirps estimates better the average total rainfall over that period but not the number of raindays
-If we look at the monthly graphs below what we we should observe is that the biases for the months in the months June - November should be lower for total rainfall in the stations mentioned and n rain days bias should be higher in the stations mentioned to explain this trend 
-
...
for (i in seq_along(stations)) {
  ...
  ggsave(here("src" , "results", paste0("month_total_rain_", s, ".png")), width = 12, height = 6)
}
```
...
ggsave(here("src", "results", paste0("smonth_n_rain", s, ".png")), width = 12, height = 6)
}
...
```{r markov_chain_plots}
  ...
  print(g)
  ggsave(here("src","results", paste0("markov_zero.png")),
         plot = g, width = 12, height = 6)
```
...
```{r markov_chain_plots_thresh}
  ...
  g <- ggplot(dat, aes(x = s_doy_date, y = prob, colour = product, size = product)) +
    ...
    ggtitle(paste("Chance of rain"))
  print(g)
  ggsave(here("src", "results", paste0("markov_zero_threshs.png")),
         plot = g, width = 12, height = 6)
```
