...
zm <- zm$get_data_frame("merged_data")
view (zm)
...
stations <- c("quanaminthe", "thiotte", "cap_hatien", "petitionville", "jacmel", "jeremie", "les_cayes","damien") #north to south would be a better order for these 
...
#Where is total_rain_station coming from? 
...
p_syear_total <- gof_pr %>%
  mutate(p = purrr::map(data, yearly_plots, gof_col = "gof__total_rain", 
                        stat_pr = "total_rain__chirps_rain", stat_st = "total_rain__station", 
                        product_name = product),
         paths = here("daily_metadata", 
                      paste0("daily_metadata", "syear_", "total_rain", "_", product, ".png")))
