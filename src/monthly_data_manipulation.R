...
df1 <- readRDS(here( "data", "daily_data_sorted1.RDS"))view 
df1 <- zm$get_data_frame("merge")
df1 <- dplyr::filter(zm, station != "thiotte")
df1$date <- as.Date(zm$date)
...
full_data <- full_join(df1, df1_grouped, by = c("station","month"))
...
full_data2 <- full_data2 %>%
  mutate(
    chirps_rain =  ifelse(chirps_rain < chirps_thresh,0,chirps_rain)
    
  ) 
view (full_data2)
  
#this section of the code should ensure that the total rainfal for the chirps and station data are the same 

alpha_grouped <- full_data2 %>% 
  group_by(station, year, month) %>%
 mutate(t_rain_chirps = sum(chirps_rain,na.rm = TRUE),
        t_rain_station = sum(rain, na.rm = TRUE),
        ratio = ifelse(!is.na(t_rain_station) & !is.na(t_rain_chirps), t_rain_chirps/t_rain_station, NA)
        )
...
alpha_grouped <- alpha_grouped %>%
  mutate(scaled_rain = ifelse(!is.na(ratio), chirps_rain * ratio, chirps_rain))
view (alpha_grouped)
